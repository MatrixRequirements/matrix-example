...
class LabelsDashboardExample implements IPlugin {
    // ****************************************
    ...
    initServerSettings(_serverSettings: XRGetProject_StartupInfo_ListProjectAndSettings) {
    }
    updateMenu(ul: JQuery, _hook: number) {
        ...
        if (this.currentFolder != undefined) {
            ...
            let cat = LabelDashboardExampleImpl.getCatFromFolderName(this.currentFolder.id);
            //check if the current item has labels
            if(LabelsDashboardExample.canBeDisplayed(cat)) {
                ul.append(menuItem);
            }
            ...
            menuItem.click(function () {
                ...
                let LabelDashboardabilityOverview = new LabelDashboardExampleImpl(ui);
    ...
    supportsControl(ctrlType: string): boolean {
        ...
        return (ctrlType === LabelsDashboardExample.fieldType);
    }
    ...
    getProjectPages(): IProjectPageParam[] {
        ...
        {
            pages.push({
                id: "ELAO", title: "Example Labels Overview", folder:"DASHBOARDS", order:3000, icon: "fal fa-tasks-alt",  usesFilters:true,  render: (options: IPluginPanelOptions) => {
                    let LabelDashboard = new LabelDashboardExampleImpl(options.control);
...
let labelsDashboardExample = new LabelsDashboardExample();

// register the engine as plugin
$(function () {
    plugins.register(labelsDashboardExample);
...
$.fn.LabelDashboard_summary = function (this: JQuery, options: ILabelDashboardSummaryOptions) {
    ...
    let baseControl = new LabelDashboardExampleImpl(this);
...
class LabelDashboardExampleImpl extends BaseControl {
    ...
    elementsLst: XRTodo[];
    protected createHelp(_settings: IBaseControlOptions): JQuery {
    ...
    init(options: ILabelDashboardSummaryOptions) {
        if (options.controlState == ControlState.FormEdit || options.controlState == ControlState.FormView) {
            ...
            if (options.currentFolder != undefined) {
                let cat =  LabelDashboardExampleImpl.getCatFromFolderName(options.currentFolder.id);
    ...
    public renderProjectPage() {
        ...
        $("#LabelDashboardInputFilter").on("keyup", function (e) {
            var value = ($(e.target).val() as String).toLowerCase();
            $("#itemLabelDashboardList tbody tr").show();
