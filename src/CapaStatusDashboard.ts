...
class ExampleDashboard implements IPlugin {
    // ****************************************
    ...
    supportsControl(ctrlType: string): boolean {
        ...
        return ctrlType === ExampleDashboard.fieldType;
    }
...
let exampleDashboardabilityOverview = new ExampleDashboard();
...
class ExampleDashboardabilityOverviewImpl extends BaseControl {
    ...
    public SelectionChanged(cat: string, folder?: string) {
        ...
        if (!this.popupModeOrControl) that.setLastSelection(cat);
    ...
    start() {
        ...
        if (that.popupModeOrControl) return;
    ...
    renderLabelDashboardTable(needles: XRTrimNeedleItem[], cat: string) {
        ...
        if (needles != undefined) {
            needles.forEach((needle: XRTrimNeedleItem) => {
                ...
                let itemId = needle.itemOrFolderRef.substring(
                    0,
                    needle.itemOrFolderRef.lastIndexOf("-")
                );
                ...
                labels = labels.filter((l) => {
                    return that.currentLabels.find((oo) => oo.label == l);
                });
    ...
    public renderProjectPage() {
        ...
        categories.forEach((cat) => {
            if (ml.LabelTools.getLabelDefinitions([cat]).length > 0) {
                let item = $(
                    `<li class="cat" data-cat="${cat}"><a href="javascript:void(0)">${cat}</a></li>`
                ).click(function () {
                    that.SelectionChanged(cat);
                });
    ...
    installCopyButtons(title: string) {
        ...
        ml.UI.copyBuffer(
            ...
            (copied: JQuery) => {
                ...
                $("#LabelDashboardPieChartTitle", copied).html(
                    "<h1>" +
                        title +
                        "</h1> <span> <b> Date:</b> " +
                        ml.UI.DateTime.renderCustomerHumanDate(new Date()) +
                        "<br/>"
                );
            },
            "",
            () => {
                $("#LabelDashboardContentPanel svg").each((i, item) => {
                    saveSize.push($(item).width());
                });
                that.charts.forEach((chart) => {
                    chart.resize({ width: 590 });
                });
                /*()=>{
        ...
        let params: c3.ChartConfiguration = {
            ...
            data: {
                ...
                onclick: function (d, i) {
                    setTimeout(() => {
                        that.filterByLabel({
                            type: colums[colums.length - 1][d.x + 1],
                        });
                    }, 100);
                },
                x: "x",
    ...
    private drawChart(inputData: ILabelDashboardGraphData) {
        ...
        that.currentGroups.forEach((group) => {
